...
class Profile(models.Model):
    ...
    def generate_thumbnail(self):
        ...
        if self.image:
            ...
            # 다시 첫번째 위치로 이동 ( 밑에 있는 InMemoryUploadedFile에서 이미지를
            # 읽게 하도록 위함
            output.seek(0)

            # 장고에서 제공하는 메모리 내 이미지 파일을
            # 장고 파일 객체로 인식하게 도와주는 InMemoryUploadedFile 클래스를 이용해서
            # 이미지를 읽고, self.thumb 컬럼에 해당 변형된 이미지를 할당합니다.(아직 저장 x)
...
def create_profile(sender, instance=None, created=False, **kwargs):
    if created:
        Profile.objects.create(owner=instance, nickname='temporal nickname')
